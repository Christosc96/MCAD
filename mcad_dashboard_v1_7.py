...
def run_discord_analysis_cached(df, selected_channels):
    """Filter a cached results dataframe for subsets that are a subset or
    superset of selected_channels, mirroring mcad_generator's cached path."""
    selected = set(selected_channels)
    channel_cols = sorted(
        [c for c in df.columns if c.startswith("ch")],
        key=lambda x: int(x[2:])
    )

    def row_subset(row):
        return {i for i, c in enumerate(channel_cols) if row[c] == 1}

    all_results = []
    scores_by_arity = {}

    for _, row in df.iterrows():
        subset = row_subset(row)
        if subset.issubset(selected) or selected.issubset(subset):
            score = row["score"]
            arity = int(row["arity"])
            all_results.append({'subset': sorted(subset), 'arity': arity, 'score': score})
...
def make_channel_checkboxes(n):
    """Build a compact, user-friendly channel selector."""
    boxes = []
    for i in range(n):
        boxes.append(
            html.Div([
            ...
            ], className='channel-checkbox-card')
        )

    return html.Div([
        html.Div([
            html.Button('Select all', id='select-all-channels-btn',
                        n_clicks=0, className='channel-select-btn'),
            html.Button('Clear all', id='clear-all-channels-btn',
                        n_clicks=0, className='channel-select-btn')
        ], className='channel-selector-actions'),
        html.Div(boxes, className='channel-checkbox-grid')
    ])


def build_discord_results_div(
    all_results, mean_scores, selected_channels, status_suffix="", arity_filter="all"
):
    """Build the bar chart and top-20 table, optionally filtered by arity."""
    ...
    fig.update_layout(
        title=dict(
            text=f'Discord Profile Analysis - Channels {selected_channels}{status_suffix}',
            font=dict(size=16, weight=600)
        ),
        xaxis=dict(title='Channel Set Arity', tickmode='linear', tick0=0, dtick=1),
        yaxis=dict(title='Mean Anomaly Score'),
        plot_bgcolor='#fafbfc',
        paper_bgcolor='white',
        height=400,
    ...
    if arity_filter != 'all':
        try:
            selected_arity = int(arity_filter)
            filtered_results = [
                r for r in all_results if int(r['arity']) == selected_arity
            ]
            table_title = f'Top Scores — Arity {selected_arity}'
        except (TypeError, ValueError):
            filtered_results = all_results
            table_title = 'Top 5 Anomaly Scores'
    else:
        filtered_results = all_results
        table_title = 'Top 5 Anomaly Scores'

    table_rows = []
    for result in sorted(filtered_results, key=lambda x: x['score'], reverse=True)[:5]:
        table_rows.append(
            html.Tr([
                html.Td(
                    str(sorted(result['subset'])),
                    style={'padding': '0.5rem', 'borderBottom': '1px solid #e1e4e8'}
                ),
                html.Td(
                    str(result['arity']),
                    style={'padding': '0.5rem', 'borderBottom': '1px solid #e1e4e8',
    ...
    return html.Div([
        ...
        html.Div([
            html.H4(table_title, style={
                'fontSize': '14px', 'fontWeight': 600, 'marginBottom': '0.75rem'
            }),
            html.Table([
                html.Thead([
                    html.Tr([
                        html.Th('Channel Subset', style={
                            'padding': '0.5rem', 'borderBottom': '2px solid #24292e',
                            'textAlign': 'left'
                        }),
                        html.Th('Arity', style={
                            'padding': '0.5rem', 'borderBottom': '2px solid #24292e',
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div([
            ...
            html.Div([
                html.Label('Analysis Mode', className='control-label'),
                dcc.RadioItems(
                    id='analysis-mode',
                    options=[
                        {'label': ' Live computation', 'value': 'live'},
                        {'label': ' Cached file', 'value': 'cached'},
                    ],
                    value='live',
                    labelStyle={'display': 'block', 'marginBottom': '0.3rem', 'fontSize': '13px'}
                )
            ], className='control-group'),

            html.Div([
                dcc.Upload(
                    id='upload-cached-file',
                    children=html.Div(['Drag & drop or ', html.A('select a cached results file (.csv/.parquet)')]),
                    style={
                        'width': '100%', 'padding': '0.75rem', 'borderWidth': '1px',
                        'borderStyle': 'dashed', 'borderRadius': '6px', 'borderColor': '#d1d5da',
                        'textAlign': 'center', 'fontSize': '12px', 'color': '#586069',
                        'backgroundColor': '#fafbfc', 'cursor': 'pointer'
                    },
                    multiple=False
                ),
                html.Div(id='cached-file-status', style={'marginTop': '0.4rem', 'fontSize': '12px', 'color': '#6a737d'})
            ], className='control-group'),
            ...
            html.Div([
                html.Label('Display Arity', className='control-label'),
                dcc.Dropdown(
                    id='arity-selector',
                    options=[{'label': 'All arities', 'value': 'all'}],
                    value='all',
                    clearable=False,
                    searchable=False,
                    style={'fontSize': '13px'}
            ...
            html.Div([
                html.Label('Window Size (m) — ignored in Cached mode', className='control-label'),
...
def generate_data(n_clicks, T, N, k, discord_length, normality_coef):
    ...
    channel_checkboxes = make_channel_checkboxes(N)
...
# Callback for convenient channel selection
@app.callback(
    Output({'type': 'channel-checkbox', 'index': dash.dependencies.ALL}, 'value'),
    Input('select-all-channels-btn', 'n_clicks'),
    Input('clear-all-channels-btn', 'n_clicks'),
    prevent_initial_call=True
)
def update_channel_selection(select_clicks, clear_clicks):
    triggered = callback_context.triggered_id

    # With MATCH/ALL pattern outputs, Dash determines the number of outputs
    # from the components currently present in the layout. Therefore return
    # one value per actual checkbox, rather than using N-input (which may be
    # stale or different from the number of rendered checkboxes).
    n_checkboxes = len(callback_context.outputs_list)

    if triggered == 'select-all-channels-btn':
        return [[item['id']['index']] for item in callback_context.outputs_list]

    if triggered == 'clear-all-channels-btn':
        return [[] for _ in range(n_checkboxes)]

    return dash.no_update


# Callback to parse an uploaded cached-results file (.csv/.parquet)
@app.callback(
    Output('cached-df-store', 'data'),
    Output('cached-file-status', 'children'),
    Output('channel-selector', 'children', allow_duplicate=True),
    Input('upload-cached-file', 'contents'),
    State('upload-cached-file', 'filename'),
    prevent_initial_call=True
)
def load_cached_file(contents, filename):
    if contents is None:
    ...
    try:
        ...
        if filename.endswith('.csv'):
            ...
            return dash.no_update, f"⚠️ Unsupported file type: {filename} (use .csv or .parquet)", dash.no_update
        ...
        if not channel_cols or 'score' not in df.columns or 'arity' not in df.columns:
            return dash.no_update, "⚠️ File missing required 'chN', 'arity', or 'score' columns", dash.no_update

        n_channels = len(channel_cols)
        status = f"✓ Loaded {filename} — {len(df)} rows, {n_channels} channels detected"

        return df.to_json(orient='split'), status, make_channel_checkboxes(n_channels)

    except Exception as e:
        return dash.no_update, f"❌ Error reading file: {e}", dash.no_update


# Callback to populate the arity selector
@app.callback(
    Output('arity-selector', 'options'),
    Output('arity-selector', 'value'),

    Input('N-input', 'value'),
    Input('cached-df-store', 'data')
)
def update_arity_selector(n_channels, cached_json):
    """Populate Display Arity from cached data when available.

    cached-df-store is an Input so the selector refreshes after the upload
    callback has actually stored the newly uploaded dataframe. This prevents
    the selector from incorrectly using the live-generation channel count/k.
    """
    arities = set()

    # Cached data: use every arity actually present in the cached file.
    if cached_json is not None:
        try:
            df = pd.read_json(
                io.StringIO(cached_json),
                orient='split'
            )

            if 'arity' in df.columns:
                numeric_arities = pd.to_numeric(
                    df['arity'], errors='coerce'
                ).dropna()

                arities = {
                    int(a)
                    for a in numeric_arities.unique()
                    if int(a) > 0
                }

                print(f"Detected arities from cached file: {sorted(arities)}")

        except Exception as e:
            print(f"Could not read cached arities: {e}")

    # No cached data: live mode uses arities available for N channels.
    if not arities:
        try:
            n_channels = int(n_channels or 0)
            if n_channels > 0:
                arities = set(range(1, n_channels + 1))
        except (TypeError, ValueError):
            arities = set()

    options = [
        {'label': 'All arities', 'value': 'all'}
    ]
    options.extend(
        {'label': f'Arity {arity}', 'value': arity}
...
# Callback for discord analysis (live computation or cached file)
@app.callback(
    ...
    State('cached-df-store', 'data'),
    State('arity-selector', 'value'),
    prevent_initial_call=True
)
def run_discord_analysis(n_clicks, channel_values, m, data, mode, cached_json, arity_filter):
    ...
    if mode == 'cached':
        if cached_json is None:
            return None, "⚠️ Upload a cached results file first"

        try:
            df = pd.read_json(io.StringIO(cached_json), orient='split')
            result = run_discord_analysis_cached(df, selected_channels)
        except Exception as e:
            return None, f"❌ Error: {str(e)}"

        if not result['all_results']:
            return None, "⚠️ No matching cached subsets found for this channel selection"

        results_div = build_discord_results_div(
            result['all_results'],
            result['mean_scores'],
            selected_channels,
            status_suffix="",
            arity_filter=arity_filter
        )
        status = f"✓ Loaded from cache - {len(result['all_results'])} matching subsets"
    ...
    try:
        ...
        results_div = build_discord_results_div(
            all_results, mean_scores, selected_channels,
            arity_filter=arity_filter
        )
...
# Callback to disable Window Size input when in Cached mode (it's unused there)
@app.callback(
    Output('analysis-m-input', 'disabled'),
    Input('analysis-mode', 'value')
)
def toggle_m_input(mode):
    return mode == 'cached'
